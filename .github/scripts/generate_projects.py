#!/usr/bin/env python3
"""
Generate an animated, theme-matched projects panel (projects.svg / projects-light.svg).
Reads projects.json and compiles a 2-column grid of mini terminal cards matching the profile banner.
"""
...
def card(p, x, y, idx):
    ...
    # Last Updated info (Stars omitted safely)
    a(f'<text x="68" y="155" font-size="11" fill="{MUTED}">'
      f'<tspan fill="{DIM}">updated {rel_time(p.get("pushed_at"))}</tspan></text>')
...
if __name__ == "__main__":
    ...
    for theme, fname in (("dark", "projects.svg"), ("light", "projects-light.svg")):
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write(svg)
        print(f"Generated projects panel -> {path} ({theme})")