...
def get_image_path(filename):
    # Check script directory first, then root repository directory
...
def get_shape_coords(image_name, invert=False, resize=(200, 200)):
    try:
        ...
        # Handle transparent backgrounds by pasting over solid white
        ...
        # Failsafe if logo is entirely white/invisible
        if len(coords) == 0:
        ...
        # Pad coordinates if the logo is extremely small
        ...
        # Return a valid fallback geometric shape so the SVG never breaks
        return np.column_stack((np.linspace(50, 250, NUM_TRAVELERS), np.ones(NUM_TRAVELERS) * 100)).astype(int)
...
def generate_svg(theme="dark"):
    ...
    try:
        ...
        # Support both .jpg and .png portrait formats
        orig_path = get_image_path("portrait.jpg")
        if not os.path.exists(orig_path):
            orig_path = get_image_path("portrait.png")
            
        orig_img = Image.open(orig_path).convert("L").resize((300, 340), Image.Resampling.LANCZOS)
        ...
        if len(px) > 0:
            portrait_path = " ".join([f"M{x+GRID_OFFSET_X} {y+GRID_OFFSET_Y}h1" for x, y in zip(px, py)])
            svg.append(f'<path d="{portrait_path}" stroke="{portrait_color}" stroke-width="1" shape-rendering="crispEdges">')
            svg.append(f'  <animate attributeName="opacity" values="1;1;0;0;0;0;1" keyTimes="0;0.21;0.28;0.88;0.93;0.96;1" dur="14.2s" repeatCount="indefinite"/>')
            svg.append('</path>')
    except Exception as e:
    ...
    path_2 = " ".join([f"M{x+GRID_OFFSET_X} {y+GRID_OFFSET_Y}h1" for x, y in l_coords_ordered])
